...
- **next-themes**: 다크모드 지원 — ⏳ **미설치. Phase 5에서 설치** (FR-U02)
- **prettier-plugin-tailwindcss**: 자동 클래스 정렬 — ⏳ **미설치** (도입 여부 미확정)

> 아래 `next-themes` 예시는 Phase 5에서 설치한 뒤 적용한다.
...
> ⚠️ **이 프로젝트는 Tailwind v4 + shadcn/ui의 OKLCH 토큰을 사용합니다.** HSL 3값 표기(`0 0% 100%`)가 아닙니다.
> 아래는 발췌이며, **실제 값의 단일 출처는 `app/globals.css`** 입니다. 토큰을 추가·변경할 때는 그 파일을 직접 확인하십시오.

`app/globals.css`의 `:root`(라이트 모드) 발췌 — 채도 0의 완전 무채색 스케일로 PRD FR-U01(뉴트럴 미니멀)을 구현합니다:
...
다크 모드는 `.dark` 블록에 같은 토큰을 재정의합니다(`@custom-variant dark (&:is(.dark *))`).
`@theme inline` 블록이 이 변수들을 `--color-*`로 노출하므로, 유틸리티에서는 `bg-background`·`text-foreground`처럼 사용합니다.
