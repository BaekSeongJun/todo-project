...
전체 모노레포 구조와 백엔드 제약은 [PRD.md 10장(제약 조건)](../PRD.md)을, 라우트 정의는 [PRD.md 6장(화면 정의)](../PRD.md)을 기준으로 합니다.
...
├── docs/                     # 📚 프로젝트 문서
│   ├── PRD.md               # 무엇을 만들 것인가 (스택·API·데이터 모델의 단일 출처)
...
│                                #   ⚠️ 생성·수정은 별도 페이지가 아니라
│                                #      이 페이지 내 다이얼로그로 처리한다 (PRD 6장)
└── oauth/callback/page.tsx       # OAuth 콜백 (일회용 코드 수신 → 교환 API 호출)
                                  #   ⚠️ 경로는 /oauth/callback (PRD 6장). /oauth2/callback 아님
                                  #   ⚠️ JWT가 아니라 일회용 코드를 받는다 (PRD FR-A08)
```
