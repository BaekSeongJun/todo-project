...
이 프로젝트의 불변 규칙([PRD.md](../PRD.md) **FR-A02**)은 **비밀번호 최소 6자**이며,
...
**데이터 영속화·인증 부분은 전부 [PRD 8장 API 요약](../PRD.md#8-api-요약)의 엔드포인트 호출로 대체**합니다.
응답 형식(`ApiResponse<T>` / `PageResponse<T>` / 에러 구조)은 **[PRD 8.1 응답 형식](../PRD.md#81-응답-형식)이 단일 출처**입니다.
...
```bash
# ❌ 미설치 — PRD 1.3 기준 Phase 6에서 설치한다.
#    설치 전에는 이 패키지들을 import 하지 말 것 (빌드 실패).
#    설치 후 PRD 1.3 표의 설치 상태를 ✅로 갱신할 것.
...
export const registerSchema = z
  .object({
    // ⚠️ PRD FR-A01-1: 이름은 필수, 1~50자. 백엔드 @Size(min = 1, max = 50)과 일치시킨다.
    name: z
      .string()
      .min(1, '이름을 입력해주세요')
...
export function LoginForm() {
  ...
  const mutation = useMutation({
    ...
    onError: (error: unknown) => {
      // 필드 검증 실패면 에러 응답의 errors[]에 field/message 쌍이 담겨 온다 (PRD 8.1)
      ...
      // 자격 불일치 등 필드로 귀속되지 않는 오류는 폼 전체 에러로 표시
...
| 중복 이메일 검사 | 프론트의 `getUserByEmail()` | 백엔드 → 409 응답 (FR-A03) |
| 세션 | 쿠키 세션 | JWT (24h) 클라이언트 저장 |
| 제출 트리거 | `useActionState` + `form action` | React Query `mutation` |
| 에러 표시 | Action 반환값 | 에러 응답의 `code` / `errors[]` 필드 에러 맵 |
